...
export default function TransactionsPage() {
    ...
    const handleExport = async (format: 'excel' | 'csv' | 'json' = 'csv') => {
        ...
        try {
            ...
            await exportViaApi('/transactions/export', params, `my_transactions_${Date.now()}`);
            const formatLabels: Record<string, string> = { excel: 'Excel (.xlsx)', csv: 'CSV', json: 'JSON' };
            toast({ title: 'Export Ready', description: `Full transaction history exported to ${formatLabels[format]}.` });
        } catch (err: any) {
            if (processedTransactions.length > 0) {
                exportTransactions(processedTransactions, { filename: 'transactions', format, sheetName: 'Transactions' });
                toast({ title: 'Export Downloaded', description: `Exported ${processedTransactions.length} displayed transactions.` });
            } else {
    ...
    const fetchTransactions = useCallback(async () => {
        ...
        try {
            ...
            setTransactions(data as OrderTransaction[]);
        } catch (err) {
    ...
    // Socket listeners for real-time transaction updates
    useEffect(() => {
        if (!socket) return;

        socket.on('transactionUpdate', (data: { id: string; status: string; message: string }) => {
            fetchTransactions();
            toast({
            ...
            if (selectedOrder && selectedOrder.id === data.id) {
                // Fetch the updated one inline or just refresh
                fetchTransactions();
            }
        });
    ...
    }, [socket, fetchTransactions, toast, selectedOrder]);

    const handleSync = async (id: string) => {
        setSyncingId(id);
        try {
            const result = await transactionService.sync(id);
            if (result.synced) {
                toast({
                    title: 'Status Updated',
                    description: `Your order status is now ${result.newStatus || 'updated'}.`
                });
                fetchTransactions();
            } else {
                toast({
                    title: 'Status Up to Date',
                    description: result.message || 'The status matches the provider.'
                });
            }
        } catch (err) {
            ...
            toast({
                title: 'Sync Failed',
                description: 'Could not sync status. Try again later.',
                variant: 'destructive'
            });
        } finally {
            setSyncingId(null);
    ...
    const processedTransactions = useMemo(() => {
        let result = transactions.filter(tx => {
            ...
            const matchesStatus = statusFilter === 'all' ||
                (statusFilter === 'processing' && (tx.status === 'processing' || tx.status === 'pending' || tx.status === 'ongoing' || tx.status === 'queued')) ||
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 p-1">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="font-display text-3xl font-black tracking-tight text-foreground">Transactions</h1>
                        <p className="text-muted-foreground font-medium">View all your transaction history</p>
                    </div>
                </div>
                <div className="flex gap-2">
                    <Button variant="outline" size="sm" onClick={fetchTransactions} className="rounded-xl border-border/50 font-bold">
                                    ...
                                    processedTransactions.map((tx, index) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                {visibleColumns.size && (
                                                    <td className="p-4 text-sm font-semibold">{tx.dataAmount}</td>
                                                )}
                                                {visibleColumns.recipient && (
                                                    <td className="p-4 text-sm font-medium">{tx.recipientPhone}</td>
                                                )}
                                                {visibleColumns.network && (
                                                ...
                                                {visibleColumns.status && (
                                                    <td className="p-4 text-sm">
                                                        <span className={cn(
                                                            ...
                                                            {tx.status === 'pending' ? 'processing' : tx.status === 'pending_payment' ? 'pending payment' : tx.status}
                                                        </span>
                                                    </td>
                                                )}
                                                {visibleColumns.source && (
                                                ...
                                                {visibleColumns.amount && (
                                                    <td className="p-4 font-mono text-sm font-extrabold text-foreground">{formatGHS(tx.amount)}</td>
                                                )}
                                                {visibleColumns.balAfter && (
                                                    <td className="p-4 font-mono text-xs hidden xl:table-cell text-muted-foreground">{formatGHS(tx.balanceAfter)}</td>
                                                )}
                                                {visibleColumns.date && (
                                                    <td className="p-4 text-xs text-muted-foreground hidden sm:table-cell">{formatDate(tx.createdAt)}</td>
                                                )}
                                                {visibleColumns.updated && (
